...
今回のデモを通して、AIMの利点だと感じた点をまとめます。

### 対話とダッシュボードの役割分担が気持ちいい

接続、移行対象の選定、抽出方式、テーブル形式といった判断は、すべてCoCoとの対話で指定できました。一方で「今どこまで進んでいるか」はダッシュボードで俯瞰でき、オブジェクト単位の依存関係まで追えます。

考えることは対話で、状況確認はダッシュボードで、と自然に分かれるので、移行作業中に迷子になりません。プロジェクト管理用の進捗表を別途作る必要がないのも実務では効いてくるのではないかと考えています。
...
必要な精度に応じてレベルを選べるので、「まずL1とL2で全体を確認し、重要なテーブルだけL3で行レベルまで突き合わせる」といった進め方ができます。移行プロジェクトで一番気になるデータの整合性確認を、ツール側の仕組みに載せられるのは大きな安心材料です。

対応する型の範囲は次のドキュメントで確認できます。

https://docs.snowflake.com/en/migrations/aim-for-datawarehouses/data-migration-validation/validate-redshift

### プロシージャとビューまで含めて回せる

AIMは依存関係を解析して移行順序（Wave）を組むため、テーブルとそれを参照するビューやプロシージャを、正しい順番でまとめて移行できます。加えて、プロシージャや関数は移行元の出力をベースラインとして取得し、移行後の結果と突き合わせるテストの仕組みも備えています。

一度行った修正を再利用可能なルールとして抽出し、プロジェクト全体へ展開する機能もあるため、オブジェクト数が多い移行ほど後半が楽になる設計です。小さく試して仕組みを掴んでから、本番規模へ広げていけます。
